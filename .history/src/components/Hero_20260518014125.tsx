      ...
      <div className="mx-auto w-full max-w-[1440px]">
        ...
        >
          ...
          <span className="inline-flex min-h-[1.35em] items-center overflow-visible py-0.5">
            <LiveTimeTicker location="Vancouver" timeZone="America/Vancouver" />
          </span>
        </motion.p>

        <h1 className="font-display text-[clamp(3.5rem,12vw,12rem)] font-semibold leading-[0.9] tracking-tight">
          {lines.map((word, i) => (
            <span key={word} className="block overflow-hidden pb-[0.15em] -mb-[0.15em]">
              <motion.span
                initial={{ y: "110%" }}
                animate={{ y: 0 }}
                ...
                className="block"
              >
                {(word === "reason." ? "reason." : word).split("").map((char, j) => {
        ...
        >
          <p className="max-w-md text-base text-foreground/70 leading-relaxed">
            Jisoo Jang — product designer crafting calm, considered interfaces.
            I believe good design is effortless and works naturally.
          </p>
