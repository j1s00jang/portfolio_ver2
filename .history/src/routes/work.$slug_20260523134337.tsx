    ...
    for (const block of blocks) {
        if (typeof block === "string") {
            ...
            out.push(
                ...
                >
                    <span className="rounded-sm bg-accent px-1.0 py-1.0 font-semibold text-accent-foreground [box-decoration-break:clone] md:px-1 md:py-2 [&_strong]:text-accent-foreground">
...
function CaseStudySectionBody({
    ...
    if (blocks?.length) {
        const hasStatsRow = blocks.some(
            (b) => typeof b === "object" && b !== null && "stats" in b,
        );
        return (
            <div
                className={`space-y-5 text-lg leading-relaxed text-foreground/85 ${
                    hasStatsRow ? "max-w-5xl" : "max-w-3xl"
                }`}
            >
