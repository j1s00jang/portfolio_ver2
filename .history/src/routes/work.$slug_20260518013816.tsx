...
export const Route = createFileRoute("/work/$slug")({
  ...
  head: ({ loaderData }) => ({
    meta: [
      ...
      { property: "og:title", content: `${loaderData?.title ?? "Project"} — Jisoo Jang` },
  ...
  notFoundComponent: () => (
    <div className="flex min-h-screen items-center justify-center px-6">
      <div className="text-center">
        <p className="font-mono text-xs uppercase tracking-[0.2em] text-muted-foreground mb-4">404</p>
...
function ProjectPage() {
  ...
  return (
    <main className="min-h-screen bg-background text-foreground">
      ...
      <section id="intro-visuals" className="px-6 md:px-10 pb-24 md:pb-32">
            ...
            <section key={s.id} id={s.id} className="scroll-mt-24">
              ...
              {s.id === "overview" ? (
                <div className="space-y-5 text-lg leading-relaxed text-foreground/85 max-w-3xl">
                  {project.overview?.map((p: string, i: number) => <p key={i}>{p}</p>)}
                </div>
              ) : (
